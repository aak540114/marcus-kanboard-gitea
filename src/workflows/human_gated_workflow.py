...
class HumanGatedWorkflow:
    ...
    def _worker_instructions() -> str:
        ...
        return (
            ...
            "redo it from scratch. ALSO check `context.recent_comments` for "
            "a '🐛 Bugs Found by AI Verify' comment — if one is there, those "
            "listed issues are required fixes, not optional; address every "
            "one of them before calling signal_ready_for_review again.\n"
    ...
    async def get_work_context(
        ...
        return {
            ...
            "instructions": (
                ...
                "3. Read the description and acceptance_criteria. Also check "
                "recent_comments for a '🐛 Bugs Found by AI Verify' comment — "
                "if one is there, those listed issues are required fixes, "
                "not optional; address every one of them.\n"
