...
class TestEnableForMarcus:
    """--enable-for-marcus opts the provisioned project into Marcus's scope.

    Marcus reads only the boards it has been enabled for, and nothing else
    in setup enables anything — so without this a freshly-provisioned
    stack watches NO board at all: agents poll forever and every ready,
    assigned ticket is invisible until a human finds the board-header
    toggle. The project setup.sh creates exists specifically for Marcus,
    so it is opted in here. Projects a human creates later still default
    to OFF.
    """

    def test_enables_the_provisioned_project(self, tmp_path, capsys):
        """The flag marks exactly the provisioned project as enabled."""
        ...
        with patch("provision_kanboard.find_or_create_project", return_value=5), patch(
            ...
            rc = pk.main(
                [
                    ...
                    "--token", "tok",
                    "--project-name", "P",
                    "--enable-for-marcus", str(tmp_path),
                ]
            )

        assert rc == 0
        # stdout stays exactly the bare project id — setup.sh captures it.
        assert capsys.readouterr().out.strip() == "5"
        assert ProjectAccessSettingManager(data_dir=tmp_path).enabled_project_ids() == [5]

    def test_an_explicit_disable_survives_a_re_run(self, tmp_path, capsys):
        """Re-running setup must NOT resurrect a project the human turned off.

        teardown.sh deletes nothing, so data/project_access_settings.json
        carries the enabled/disabled state across a teardown+setup cycle.
        Force-enabling on every setup run would silently undo a deliberate
        "Marcus: OFF" the next time the stack is rebuilt.
        """
        from src.core.project_access_settings import ProjectAccessSettingManager

        ProjectAccessSettingManager(data_dir=tmp_path).set_project_enabled(5, False)
        ...
        assert mgr.get_project_enabled(5) is False
        assert mgr.enabled_project_ids() == []

    def test_an_explicit_enable_is_left_alone(self, tmp_path):
        """An already-enabled project stays enabled (and is not rewritten)."""
        from src.core.project_access_settings import ProjectAccessSettingManager

        ProjectAccessSettingManager(data_dir=tmp_path).set_project_enabled(5, True)
    ...
    def test_without_the_flag_nothing_is_enabled(self, tmp_path, capsys):
        """Default stays hands-off: no settings file is written."""
        from src.core.project_access_settings import ProjectAccessSettingManager

        with patch("provision_kanboard.find_or_create_project", return_value=5), patch(
            "provision_kanboard.reconcile_columns", return_value=[]
        ):
            pk.main(["--url", "http://x/jsonrpc.php", "--token", "tok", "--project-name", "P"])

        assert ProjectAccessSettingManager(data_dir=tmp_path).enabled_project_ids() == []

    def test_enable_failure_does_not_fail_provisioning(self, tmp_path, capsys):
        """A settings-write failure warns but still exits 0 — the board
        itself was provisioned fine, and setup.sh must not abort."""
        with patch("provision_kanboard.find_or_create_project", return_value=5), patch(
            "provision_kanboard.reconcile_columns", return_value=[]
        ), patch(
            "src.core.project_access_settings.ProjectAccessSettingManager.set_project_enabled",
        ...
        assert "read-only fs" in out.err or "could not" in out.err.lower()
