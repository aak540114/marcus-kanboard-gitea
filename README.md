...
**Every project starts disabled**, with one exception: the project `scripts/setup.sh` creates is enabled automatically, since it exists specifically for Marcus — a fresh install works end to end with no toggling. Every *other* project does nothing — no repo, no columns, no claimed tickets, no agent commits — until a human explicitly opts it in. Open that project's board in Kanboard and click the **"🔒 Marcus: OFF for this project"** button in the header (it's the first control, to the left of the active-agents badge); it flips to **"🔓 Marcus: ON for this project"** and Marcus provisions the repo + columns immediately (no waiting on the backstop poll).

A few things worth knowing:
- **Marcus only READS the boards it is enabled for.** The setting is not just a veto on tickets it already saw — it scopes which projects get polled at all. Enabling a project puts it in scope on the next poll (no restart, no config change); with nothing enabled Marcus reads no board, and `marcus_work` says so explicitly rather than reporting a bare "no tickets are ready".
...
- **The ON/OFF state survives a teardown.** It lives in `data/project_access_settings.json`, and `scripts/teardown.sh` deletes nothing — so `teardown.sh` followed by `setup.sh` comes back up with exactly the projects you had enabled. Setup only enables its own project when that project has *never* been configured; a project you deliberately switched OFF stays OFF across rebuilds.
